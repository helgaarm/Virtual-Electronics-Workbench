...
5. Add structured device metadata: identity, category, package, supply range, model level, limitations, sources, and tested examples. Keep the architecture open to multiple model levels without adding premature UI controls.
   Assign the component a folder and icon in `src/ui/paletteCatalog.ts`. The exhaustive component record requires every kind to appear in the parts drawer; search includes its display name, kind, description, and folder name.
