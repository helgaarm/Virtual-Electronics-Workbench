...
const roundPad = (number: string, terminalId: string, xMm: number, yMm: number, drillDiameterMm = 0.8, diameterMm = 1.8) => ({
  number, terminalId, positionMm: { xMm, yMm }, sizeMm: { widthMm: diameterMm, heightMm: diameterMm },
...
export const PCB_FOOTPRINTS: Readonly<Record<string, PcbFootprint>> = {
  ...
  'CP-Radial-D5-P2.0': { id: 'CP-Radial-D5-P2.0', name: 'Radial capacitor, 5 mm', compatibleKinds: ['capacitor'], bodySizeMm: { widthMm: 5, heightMm: 5 }, courtyardMarginMm: 1, pads: [roundPad('1', 'positive', -1, 0, 0.8, 1.7), roundPad('2', 'negative', 1, 0, 0.8, 1.7)], verified: true, source: 'Generic 5 mm radial body with 2.0 mm lead pitch and 1.7 mm pads; verify against selected part.', pin1Marked: true },
